...
contract("EglVotingV2Tests", (accounts) => {
    [
        _deployer, 
        _genesisOwner, 
        _seedAccount1, 
    ...
    beforeEach(async () => {             
        ...
        eglContractInstance = await EglContractV2.new();        
    ...
    describe("Map Pool Reward Receivable Address", function () {
        it("should not be able to map 0 address", async () => {
            await expectRevert(
                ...
                "EGL:INVALID_DESIGNATED_ADDR"
            )
        });
        it("should map rewards receivable address to miner/pool address", async () => {
            let txReceipt = await eglContractInstance.mapMinerAddress(_voter1, { from: _coinbase });
            ...
            let mappedAddress = await eglContractInstance.minerAddresses(_coinbase)
        ...
        it("should overwrite previous mapped value if called again", async () => {
            await eglContractInstance.mapMinerAddress(_voter1, { from: _coinbase });
            let mappedAddress = await eglContractInstance.minerAddresses(_coinbase)
            assert.equal(mappedAddress, _voter1, "Incorrect mapped address");
            await eglContractInstance.mapMinerAddress(_genesisSupporter1, { from: _coinbase });
            let updatedMappedAddress = await eglContractInstance.minerAddresses(_coinbase)
            assert.equal(updatedMappedAddress, _genesisSupporter1, "Mapped address not updated");
        });
        it("should allow changing of the mapped address to receive EGL's at the new address", async () => {
            ...
            assert.equal(
                ...
                "0",
                "Incorrect designated address balance"
            );
            ...
            assert.equal(
                web3.utils.fromWei(await eglContractInstance.minerBalances(_coinbase)),
                "500",
                "Coinbase balance should still be 500"
            );
            assert.approximately(
                parseFloat(web3.utils.fromWei(await eglContractInstance.minerBalances(_proxyAdmin))),
                500,
                0.1,
                "Incorrect balance for new designated address"
            );
        });
    });
