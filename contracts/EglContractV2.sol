...
contract EglContractV2 is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /* New variables */
    mapping(address => address) public minerDelegateAddresses;
    bool public poolRewardStopped;
    ...
    event PoolRewardStopped(address sender, bool stopped, uint date);
    event BlockRewardCalculatedV2(
        uint blockNumber, 
        uint16 currentEpoch,
        uint remainingPoolReward,
        int blockGasLimit, 
    ...
    */
     ...
     */
    function sweepPoolRewards(address _delegateAddress) external whenNotPaused {
        require(!poolRewardStopped, "EGL:REWARDS_STOPPED");
        require(block.number % 100 == 0, "EGL:NOT_REWARD_BLOCK");
        require(block.number > latestRewardSwept, "EGL:ALREADY_SWEPT");
        require(_delegateAddress != address(0), "EGL:NO_DELEGATE_ADDRESS");
        require(
            _delegateAddress == minerDelegateAddresses[block.coinbase] || 
            _delegateAddress == block.coinbase, 
            "EGL:DELEGATE_NOT_RECOGNIZED"
        );
        ...
        if (blockReward > 0) {
            ...
            bool success = eglToken.transfer(_delegateAddress, Math.umin(eglToken.balanceOf(address(this)), blockReward));
            require(success, "EGL:TOKEN_TRANSFER_FAILED");
        }
    ...
    /**
     * @notice Owner only function to pause contract
     */
     ...
     * 
     * @param _delegateAddress Address that will receive reward EGL's
     */
    function mapDelegateAddress(address _delegateAddress) external {
        require(_delegateAddress != address(0), "EGL:INVALID_DELEGATE_ADDR");
        minerDelegateAddresses[msg.sender] = _delegateAddress;
        emit MinerAddressMapped(msg.sender, _delegateAddress, block.timestamp);
    }

    function stopPoolReward(bool _stopped) external onlyOwner {
        poolRewardStopped = _stopped;
        emit PoolRewardStopped(msg.sender, _stopped, block.timestamp);
    }
        ...
        returns (uint blockReward) 
    {
        ...
        uint totalRewardPercent;
        if (delta <= 100000)
            totalRewardPercent = 100;
        else if (delta <= 250000)
            totalRewardPercent = 85;
        else if (delta <= 500000)
            totalRewardPercent = 70;
        else if (delta <= 1000000)
            totalRewardPercent = 25;

        blockReward = remainingPoolReward.mul(20000000000000)
            .mul(totalRewardPercent)
            .div(100)
            .div(DECIMAL_PRECISION);

        emit BlockRewardCalculatedV2(
            block.number,
            currentEpoch,
            remainingPoolReward,
            _blockGasLimit,
